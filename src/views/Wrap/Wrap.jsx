...
import { getBrickTokenImage, getTokenImage, trim, formatCurrency } from "../../helpers";
import { changeApproval, changeWrap } from "../../slices/WrapThunk";
import {
  changeMigrationApproval,
  bridgeBack,
  migrateWithType,
  migrateCrossChainWSBRICK,
...
function Wrap() {
  ...
  const sBrickPrice = useSelector(state => {
    return state.app.marketPrice;
  });

  const wsBrickPrice = useSelector(state => {
  ...
  const migrateWsbrickAllowance = useSelector(state => {
    return state.account.migration && state.account.migration.wsbrick;
  });

  const unwrapGbrickAllowance = useSelector(state => {
    return state.account.wrapping && state.account.wrapping.gBrickUnwrap;
  });
  ...
  const setMax = () => {
    ...
    if (assetFrom === "wsBRICK") setQuantity(wsbrickBalance);
    if (assetFrom === "gBRICK") setQuantity(gbrickBalance);
  };
  ...
  const unWrapWSBRICK = async () => {
    // eslint-disable-next-line no-restricted-globals
    if (isNaN(quantity) || Number(quantity) === 0 || quantity === "") {
      // eslint-disable-next-line no-alert
      return dispatch(error("Please enter a value!"));
    }
    if (ethers.utils.parseUnits(quantity, "ether").gt(ethers.utils.parseUnits(wsbrickBalance, "ether"))) {
  ...
  const hasCorrectAllowance = useCallback(() => {
    ...
    if (assetFrom === "wsBRICK" && assetTo === "gBRICK") return migrateWsbrickAllowance > wsbrickBalance;
    if (assetFrom === "wsBRICK" && assetTo === "sBRICK") return unwrapAllowance > wsbrickBalance;
    if (assetFrom === "gBRICK") return unwrapGbrickAllowance > gbrickBalance;
  ...
  // currentAction === "Unwrap" ? (quantity * wsBrickPrice) / sBrickPrice : (quantity * sBrickPrice) / wsBrickPrice;
  ...
  const chooseCorrectWrappingFunction = () => {
    if (assetFrom === "sBRICK" && assetTo === "gBRICK") migrateToGbrick("sbrick");
    if (assetFrom === "wsBRICK" && assetTo === "gBRICK") migrateToGbrick("wsbrick");
    if (assetFrom === "gBRICK" && assetTo === "sBRICK") unwrapGbrick();
    if (assetFrom === "wsBRICK" && assetTo === "sBRICK") unWrapWSBRICK();
  };
            ...
            <Grid item>
              <div className="card-header">
                ...
                <Link
                  ...
                  href={
                    assetTo === "wsBRICK"
                      ? "https://docs.olympusdao.finance/main/contracts/tokens#wsbrick"
                      : "https://docs.olympusdao.finance/main/contracts/tokens#gbrick"
                  }
                  aria-label="wsbrick-wut"
                  target="_blank"
                >
                ...
                <Grid container spacing={2} alignItems="flex-end">
                  <Grid item xs={12} sm={4} md={4} lg={4}>
                    <div className="wrap-sBRICK">
                      ...
                      <Typography variant="h4">
                        {sBrickPrice ? formatCurrency(sBrickPrice, 2) : <Skeleton width="150px" />}
                      </Typography>
                    </div>
                  </Grid>
                  <Grid item xs={12} sm={4} md={4} lg={4}>
                  ...
                  <Grid item xs={12} sm={4} md={4} lg={4}>
                    <div className="wrap-wsBRICK">
                      ...
                      <Typography variant="h4">
                        {wsBrickPrice ? formatCurrency(wsBrickPrice, 2) : <Skeleton width="150px" />}
                      </Typography>
                    </div>
                  </Grid>
                </Grid>
                    ...
                    {!isAvax ? (
                      <>
                        ...
                        <div className="data-row">
                          ...
                          <Typography variant="body1">
                            {isAppLoading ? <Skeleton width="80px" /> : <>{trim(wsbrickBalance, 4)} wsBRICK</>}
                          </Typography>
                        </div>
                        <div className="data-row">
                          <Typography variant="body1">gBRICK Balance</Typography>
                          <Typography variant="body1">
                            {isAppLoading ? <Skeleton width="80px" /> : <>{trim(gbrickBalance, 4)} gBRICK</>}
                          </Typography>
                        </div>
